...
int map_add(struct map *m,const char *key,void *data)
{
	unsigned char v;
	char x,is_found;
	struct map_node *node,*t,*tmp_list_head=NULL;
	int rs=0;
	...
	for(int n=0;n<m->length;n++){
		...
		// 添加到list
		if(NULL!=m->list_head) m->list_head->list_prev=t;
...
void map_del(struct map *m,const char *key,map_del_func_t fn)
{
	...
	struct map_node *node=m->tree_root,*t=m->tree_root,*tmp_list=NULL;
