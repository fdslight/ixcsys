...
import ixc_syscore.tftp.handlers.sys_proxy as sys_proxy
...
def __start_service(debug):
    if not debug and os.path.isfile(PID_FILE):
        print("the ixc_sys_proxy_main process exists")
        return
...
class service(dispatcher.dispatcher):
    ...
    def init_func(self, debug):
        ...
        self.__debug = debug
        self.__conf_path = "%s/sys_proxy.ini" % os.getenv("IXC_MYAPP_CONF_DIR")
        self.__sys_proxy_fd = -1
        self.__sys_proxy_fd6 = -1
        self.__sessions = {}
    ...
    def myloop(self):
        if self.__sys_proxy_fd > 0: self.get_handler(self.__sys_proxy_fd).loop()
        if self.__sys_proxy_fd6 > 0: self.get_handler(self.__sys_proxy_fd6).loop()
        if self.__sessions:
    ...
    def release(self):
        ...
        if self.__sys_proxy_fd > 0: self.delete_handler(self.__sys_proxy_fd)
        if self.__sys_proxy_fd6 > 0: self.delete_handler(self.__sys_proxy_fd6)


def main():
    __helper = "ixc_syscore/sys_proxy helper: start | stop | debug"
    if len(sys.argv) != 2:
        print(__helper)
        return
